...
import { copyDirIfWritable, isDryRun, removeIfWritable, writeFileIfWritable } from "../core/fsguard.js";
...
export const codexAdapter: Adapter = {
  ...
  async activate(ctx: AdapterContext): Promise<InstallReport> {
    ...
    // 上次安装记录内的目标归本工具管理,直接覆盖;记录之外的用户目录先备份
    const prevInstalled = new Set(installedFilesFor(preset.id, "codex"));
    ...
    if (!isDryRun()) {
      ...
      if (missing.length > 0) {
        return {
          ...
          presetId: preset.id,
          ok: false,
          message: `Preset '${preset.id}' self-check failed for Codex CLI — missing after install: ${missing.join(", ")}`,
          changed: all,
        };
      }
    }
    ...
    return {
      ...
      ok: true,
      message: all.length
        ? `Installed preset '${preset.id}' into Codex CLI (${all.length} changes). Restart codex to apply.${backupText}`
        : `Preset '${preset.id}' already active for Codex CLI.${backupText}`,
      changed: all,
    };
  },
