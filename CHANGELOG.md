...
Competing guardrail projects (agent-guard, oh-my-harness, claude-guardrails) each landed one good idea. This release folds the best of them into kimi-boost — while keeping what they got wrong out: no external binaries, no YAML, no orchestration engine.

**Three new guards in `core` (1.2.0):**
- `protect-guards` — self-protection. The agent can no longer disable guardrails (`kimi-boost guard --disable`) or rewrite their configuration (`~/.kimi-boost/**`, `~/.kimi-code/config.toml`, `~/.claude/settings.json`, project `.claude/`, `AGENTS.md`), including via shell redirection. You still can, from your own terminal.
- `protect-paths` — blocks hand-editing lockfiles (`package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `go.sum`, `Cargo.lock`, `poetry.lock`, `Gemfile.lock`, `composer.lock`) and writes into `node_modules/`, `dist/`, `.git/` — through file tools and shell redirection alike. Lockfiles should be regenerated by the package manager, not edited.
- `secret-scan-post` — a PostToolUse re-scan that catches secrets written via shell redirection (`echo key > .env`, `cat > f`, `tee f`), which the PreToolUse `Write`/`Edit` matchers never see. It informs (the write already happened); it does not pretend to undo.

**Modes and explanations:**
- Every guard supports `block` (default) or `warn` mode: `kimi-boost guard --warn <name>` keeps logging and reporting but stops blocking; `--block <name>` restores. `warn` is the honest alternative to competitors' interactive "ask", which most harnesses don't support anyway.
- `kimi-boost guard --explain <name>` documents what a guard blocks, how to bypass it, and the risk of disabling it.
- Every block message now prints its own escape hatch (`false positive? run: kimi-boost guard --disable <name>`).

**Same guards, in CI:**
- `kimi-boost guard --ci [--staged]` scans changed/staged files for secrets and exits non-zero with `file:line` findings — for CI pipelines and pre-commit.
- `kimi-boost guard --install-git-hook` / `--uninstall-git-hook` manage a marked, idempotent `pre-commit` block in the current repo (existing hook content is preserved).

**Honest boundaries (new README section: "What the guardrails don't promise"):**
- Pattern matching is not a security boundary — sandboxing is; obfuscated commands and runtime-assembled secrets can get through. Fail-open is deliberate. PostToolUse cannot undo. Guards are per-tool. Layer them with host-side scanning (e.g. push protection) and review.

**Deliberately not absorbed:** external runtime dependencies (gitleaks/jq/Bun), a second config language, agent-orchestration scope, or dropping Windows.
